...
const NoteList = ({searchedNotes}) => {
    ...
    const {setIsCreatingNote, setIsUpdatingNote, setIsShowSingleNote, setActive} = useContext(ActiveNoteContext);  
          ...
          {searchedNotes?.map(note => 
            <Col span={6} key={note.id}>
              ...
              >
                <Card.Meta
                  title={note.name}
                  description={<ReactMarkdown>{note.content}</ReactMarkdown>}
